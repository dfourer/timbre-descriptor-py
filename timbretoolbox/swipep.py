...
# Author: D Fourer <dominique@fourer.fr> https://fourer.fr
...
#       [p,t,s] = swipep(x,Fs,numpy.array([75, 500]),0.01,float(1./48.),1/20,0.5,0.2);
#       plot(1000*t,p)
#       xlabel('Time (ms)')
#       ylabel('Pitch (Hz)')
#
...
def swipep(x, fs,plim=numpy.array([30, 5000]),dt=0.001,dlog2p=float(1./48.), dERBs=0.1, woverlap=0.5, sTHR=-numpy.infty):
    ...
    Parameters:
        ...
        plim     : Pitch range in Hz (array-like of 2 elements)
        dt       : Time step (seconds)
        dlog2p   : Step in log2 pitch space
        dERBs    : ERB scale spacing
        woverlap : Window overlap ratio [0,1]
        sTHR     : Pitch strength threshold

    Returns:
        p        : Estimated pitch (Hz)
        t        : Time axis (seconds)
        s        : Pitch strength
    """
	fs = float(fs);
	...
	for i in range(0, len(ws)):
		dn = int(max( 1, numpy.round( 8*(1-woverlap) * fs / pO[i] ) )); # Hop size
		...
		w = numpy.hanning( int(ws[i]) ); 							# Hann window 
	...
	s = numpy.repeat( numpy.nan, numpy.shape(S)[1], 0);
