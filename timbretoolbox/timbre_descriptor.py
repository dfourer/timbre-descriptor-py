...
import timbretoolbox.swipep as swp		# used for single-F0 estimation
...
def test_descriptor():
	...
	#m = scipy.io.loadmat('param_val_ref.mat');
	#param_ref = numpy.squeeze(numpy.array(m['x']));
...
def ERBpower(a,Fs, hopsize, bwfactor=1.):
	...
	a		= numpy.concatenate( (numpy.zeros(int(offset)), a, numpy.zeros(int(wsize-offset))) );
